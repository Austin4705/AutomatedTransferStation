...
try:
    ...
    BlitzGateway = None  # type: ignore
    DatasetI = ProjectI = ImageI = None  # type: ignore
    rstring = rlong = rint = None  # type: ignore
...
class Image_Container:
    ...
    def __init__(self):
        ...
        if not OMERO_AVAILABLE:
            Logger.log("omero-py not installed — running in offline mode (image storage disabled). "
                       "Install with: pip install -e .[omero]")
            return
