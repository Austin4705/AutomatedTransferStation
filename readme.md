...
Automated Transfer Program Screenshot
...
Also ensure the proper dlls are inside of `src/ats/cameras/thor_drivers/` if you are using the thorcam. You can find more resournces at `C:\Program Files\Thorlabs\Scientific Imaging\Scientific Camera Support`
...
```bash
conda activate automatedTransfer
# Run from the project root (not src/) — the app is now a package.
PYTHONPATH=src python -m ats
...
Create a `.env` file based on `default.env` in the project root directory with the following things:
