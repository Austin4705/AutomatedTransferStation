...
class Transfer_Functions:
    ...
    def run_trace_over_command(self, data):
        ...
        for wafer in data.get("wafers", [{}]):
            ...
            self._run_trace_over_path(
                start_x, end_x, start_y, end_y, start_z, travel,
                initial_wait_time, camera, save_images, wafer_id,
                magnification, wait_time, camera_index,
            )

    @transfer_function("RUN_TRACE_OVER_AREA")
    def run_trace_over_area(self, data):
        Logger.log("Type of data: " + type(data).__name__)
        Logger.log("Data: " + str(data))
    ...
    def _run_trace_over_path(
        ...
        """Set up storage, then delegate the scan to the trace_over algorithm."""
        ...
        def store_snapshot(snap):
            if not save_images:
                return
            image_id = self.image_container.upload_image(
                snap.image, dataset_id=collection_id, image_name=f"image_{snap.sequence}"
            )
            self.image_container.apply_metadata_to_image(image_id, {"key_value_pairs": {
                "wafer_id": wafer_id,
        ...
        # Resolve via registry every call so swaps take effect.
        algo_get_active("trace_over")(
            start_x=start_x, end_x=end_x,
            start_y=start_y, end_y=end_y,
            start_z=start_z,
            step_x=travel["x"], step_y=travel["y"],
            initial_settle_time=initial_wait_time,
            point_settle_time=wait_time,
            stage=self.transfer_station,
            frames=camera,
            autofocus=lambda: self.auto_focus(camera, self.transfer_station),
            initial_autofocus=lambda: self.auto_focus(
                camera, self.transfer_station, n_samples_coarse=80, z_range_coarse=1.5
            ),
            on_frame=store_snapshot,
            should_continue=self.wait_if_paused,
            log=Logger.log,
        )

    def auto_focus(self, camera, transfer_station, n_samples_coarse=20, n_samples_fine=20, z_range_coarse=0.5, z_range_fine=0.1):
        """Run the *currently-active* autofocus variant against the given stage and camera."""
        algo_get_active("autofocus")(
            transfer_station, camera,
            n_samples_coarse=n_samples_coarse,
            n_samples_fine=n_samples_fine,
            z_range_coarse=z_range_coarse,
            z_range_fine=z_range_fine,
            should_continue=self.wait_if_paused,
            log=Logger.log,
        )
